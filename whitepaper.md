...
By dynamically acquiring resources we can ensure that the load will never exceed its capacity.
...
First `.u.subInner` is used to add the handle to `.u.w` for each table.
...
Our first option is to reduce the size of each step in capacity by reducing the size of our cluster's servers.
...
Keeping a cushion for the RDBs in the Tickerplant's queue means we will not have to worry about large spikes in demand affecting our system.

In the next section further simulations were ran to determine whether adjusting these factors is worth the risk.
